...
<body>
  ...
  <h1>Guide</h1>
  <p>Welcome to the REP Website Guide, <b><?= htmlspecialchars($username)?></b>! <?= $uid == -1 ? "(Note that signing in as Guest is view-only and turns off certain website features.)" : "" ?></p>
  ...
  <h3>Minecraft</h3>
  <p>To verify/link your Minecraft username, first join the Minecraft server with the Minecraft account you want to link.</p>
  ...
  <p>
    ...
    Each nation also always has one leader who can remove people from their nation, as well as delete the nation, but this leader cannot leave the nation.
    The nation's leader can also transfer their leadership to any member.
    When you're not in a nation, you can join any nation, or create a new nation, where you'll automatically start as the leader.
  </p>
  <p>
  ...
  <p>
    Coming soon: Changing in-game teams by Nation, changing Discord roles by Nation, automatically creating private Discord channels for each Nation, and more customization for Nations (public vs private, colors, flags, etc)
  </p>
  <h2>Discord Commands</h2>
  <p>The Discord server has some useful commands to interact with the server as well.</p>
  <p>We already mentioned /verify to verify/link your Discord account with this website.</p>
  ...
  <p>Coming soon: /status command for server status.</p>
</body>
<h2>Coming Later:</h2>
